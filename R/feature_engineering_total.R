...
f1.3 = dcast(feat.p1, Patient_ID ~ ATCLevel2Code, value.var = "ATCSkew", fun.aggregate = mean, fill = NA); colnames(f1.3) = c("Patient_ID", paste0("f1.3_", colnames(f1.3[,-1,with = F])))
...
f1.6 = dcast(feat.p1, Patient_ID ~ ATCLevel2Code, value.var = "ATCHurt", fun.aggregate = mean, fill = NA); colnames(f1.6) = c("Patient_ID", paste0("f1.6_", colnames(f1.6[,-1,with = F])))
