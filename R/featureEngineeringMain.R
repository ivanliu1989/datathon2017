...
generateHistFeatures = function(txns, lstTrans = as.Date("2016-01-01"), cores = 6){
    ...
    # res = foreach(i=as.numeric(unique(pats$cuts)), .combine = rbind) %dopar% {
        ...
        x[, drugSkew :=  moments::skewness(Dispense_Week, na.rm = T), by = .(Patient_ID, Drug_ID)]
        ...
        x[, drugHurt :=  moments::kurtosis(Dispense_Week, na.rm = T), by = .(Patient_ID, Drug_ID)]
        ...
        cat("\nFeature Sets 3...")
        x[, lstIllBuy := as.numeric(lstTrans - max(Dispense_Week))/7, by = .(Patient_ID, ChronicIllness)]
        x[, lstDrugBuy := as.numeric(lstTrans - max(Dispense_Week))/7, by = .(Patient_ID, Drug_ID)]
        x[, lstATCBuy := as.numeric(lstTrans - max(Dispense_Week))/7, by = .(Patient_ID, ATCLevel3Code)]
        x[, illLength := as.numeric(max(Dispense_Week) - min(Dispense_Week))/7, by = .(Patient_ID, ChronicIllness)]
        x[, drugLength := as.numeric(max(Dispense_Week) - min(Dispense_Week))/7, by = .(Patient_ID, Drug_ID)]
        x[, ATCLength := as.numeric(max(Dispense_Week) - min(Dispense_Week))/7, by = .(Patient_ID, ATCLevel3Code)]
