...
generateHistFeatures = function(txns, lstTrans = as.Date("2016-01-01"), cores = 6){
    ...
    x[, ATCSkew :=  moments::skewness(Dispense_Week, na.rm = T), by = .(Patient_ID, ATCLevel2Code)]
    x[, illHurt :=  moments::kurtosis(Dispense_Week, na.rm = T), by = .(Patient_ID, ChronicIllness)]
    x[, ATCHurt :=  moments::kurtosis(Dispense_Week, na.rm = T), by = .(Patient_ID, ATCLevel2Code)]
    ...
    x[, dosageATC := sum(dosage, na.rm = T), by = .(Patient_ID, Dispense_Week, ATCLevel2Code)]
    x[, dosageQtrIll := sum(dosage, na.rm = T), by = .(Patient_ID, Qtr, ChronicIllness)]
    x[, dosageQtrATC := sum(dosage, na.rm = T), by = .(Patient_ID, Qtr, ATCLevel2Code)]
    ...
    x[, dosageATCSkew :=  moments::skewness(dosageIll, na.rm = T), by = .(Patient_ID, ATCLevel2Code)]
    x[, dosageATCHurt :=  moments::kurtosis(dosageIll, na.rm = T), by = .(Patient_ID, ATCLevel2Code)]
    ### Ingredient 
    
    # generating features
    feat.p1 = copy(unique(x[, .(Patient_ID, Qtr, Dispense_Week, Drug_ID, ChronicIllness, ATCLevel2Code,
    ...
    f1.3 = dcast(feat.p1, Patient_ID ~ ATCLevel2Code, value.var = "ATCSkew", fun.aggregate = mean, fill = 0); colnames(f1.3) = c("Patient_ID", paste0("f1.3_", colnames(f1.3[,-1,with = F])))
    ...
    f1.6 = dcast(feat.p1, Patient_ID ~ ATCLevel2Code, value.var = "ATCHurt", fun.aggregate = mean, fill = 0); colnames(f1.6) = c("Patient_ID", paste0("f1.6_", colnames(f1.6[,-1,with = F])))
    ...
    f1.8 = dcast(feat.p1, Patient_ID ~ ATCLevel2Code, value.var = "dosageQtrATC", fun.aggregate = mean, fill = 0); colnames(f1.8) = c("Patient_ID", paste0("f1.8_", colnames(f1.8[,-1,with = F])))
    ...
    f1.11 = dcast(feat.p1, Patient_ID ~ ATCLevel2Code, value.var = "dosageATCSkew", fun.aggregate = mean, fill = 0); colnames(f1.11) = c("Patient_ID", paste0("f1.11_", colnames(f1.11[,-1,with = F])))
    f1.12 = dcast(feat.p1, Patient_ID ~ ATCLevel2Code, value.var = "dosageATCHurt", fun.aggregate = mean, fill = 0); colnames(f1.12) = c("Patient_ID", paste0("f1.12_", colnames(f1.12[,-1,with = F])))
    rm(feat.p1); gc()
    ...
    x[, totATCPres := length(unique(ATCLevel2Code)), by = Presc_ID]
    ...
    x[, totATCQtr := length(unique(ATCLevel2Code)), by = .(Patient_ID, Qtr)]
    
    x[, totDrugYr := length(unique(Drug_ID)), by = .(Patient_ID, Year)]
    x[, totIllYr := length(unique(ChronicIllness)), by = .(Patient_ID, Year)]
    x[, totATCYr := length(unique(ATCLevel2Code)), by = .(Patient_ID, Year)]
    ...
    x[, ATCLength := as.numeric(max(Dispense_Week, na.rm = T) - min(Dispense_Week, na.rm = T))/7, by = .(Patient_ID, ATCLevel2Code)]
    
    # generating features
    feat.p3 = copy(unique(x[, .(Patient_ID, Drug_ID, ChronicIllness, ATCLevel2Code,
                                lstIllBuy, lstDrugBuy, lstATCBuy,
                                illLength, drugLength, ATCLength)]))
    feat.p3 = repNaN(feat.p3)
    f3.1 = dcast(feat.p3, Patient_ID ~ ChronicIllness, value.var = "lstIllBuy", fun.aggregate = mean, fill = 0); colnames(f3.1) = c("Patient_ID", paste0("f3.1_", colnames(f3.1[,-1,with = F])))
    f3.2 = dcast(feat.p3, Patient_ID ~ ATCLevel2Code, value.var = "lstATCBuy", fun.aggregate = mean, fill = 0); colnames(f3.2) = c("Patient_ID", paste0("f3.2_", colnames(f3.2[,-1,with = F])))
    ...
    f3.4 = dcast(feat.p3, Patient_ID ~ ATCLevel2Code, value.var = "ATCLength", fun.aggregate = mean, fill = 0); colnames(f3.4) = c("Patient_ID", paste0("f3.4_", colnames(f3.4[,-1,with = F])))
    rm(feat.p3);gc()
