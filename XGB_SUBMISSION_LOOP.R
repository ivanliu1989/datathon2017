...
for(i in 1:5){
    ...
    write.csv(pred, file = paste0("./submit20170529/XGB_SUBMIT_AUC_SIM_COMP", xgbFit$best_score,"_",i,".csv"),
              row.names = F)
    
    save(predictors, xgbFit, file = paste0("./submit20170529/XGB_SUBMIT_AUC_SIM_COMP", xgbFit$best_score,"_",i,".RData"))
...
for(i in 21:25){
    ...
    pred = predict(xgbFit, dtesting, ntreelimit=xgbFit$bestInd)
    pred = makeSubmit(pred)
    write.csv(pred, file = paste0("./submit20170529/XGB_SUBMIT_RMSE_SIM_COMP", xgbFit$best_score,"_",i,".csv"),
              row.names = F)
    
    save(predictors, xgbFit, file = paste0("./submit20170529/XGB_SUBMIT_RMSE_SIM_COMP", xgbFit$best_score,"_",i,".RData"))
